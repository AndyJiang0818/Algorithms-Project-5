...
// Homework 5
// 4/25/21
// Modify heapsort, quicksort, and mergesort.
...
/**
 * Return middle of left, center, and right.
 * Order these and hide the pivot.
 */
template <typename Comparable, typename Comparator>
const Comparable &middlepivot(vector<Comparable> &a, int left, int right, Comparator less_than)
{
    int center = (left + right) / 2;
...
void QuickSort2(vector<Comparable> &a, int left, int right, Comparator less_than)
{
    if (left + 10 <= right)
    {
        const Comparable &pivot = middlepivot(a, left, right, less_than);
